...
[FR-05] Acceptance criteria:
  - Taiwan phone numbers are masked
  - Email addresses are masked
  - Taiwan addresses are masked
  - Sensitive keywords trigger escalation flag
...
def test_sensitive_keyword_triggers_escalation():
    """Messages containing sensitive keywords flag for escalation."""
    assert contains_sensitive_keywords("我要自殺") is True
    assert contains_sensitive_keywords("幫我報警") is True


def test_normal_message_no_escalation():
    """Normal message does not trigger escalation."""
    assert contains_sensitive_keywords("我想查詢訂單狀態") is False


def test_escalation_flag_dataclass():
    """EscalationFlag has message and keywords fields."""
    flag = EscalationFlag(message="test", keywords=["urgent"])
    assert flag.message == "test"
    assert "urgent" in flag.keywords
